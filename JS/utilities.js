...
//Get a random number between the minimum and maximum
function getRandom(min, max) {
  return Math.random() * (max - min) + min;
}

/*Get a random value from the weights
* The format of the weights must be as such
...
function getWeightedRandom(weights) {
  ...
  for (i in weights) {
    sumWeight += weights[i];
    if (randomVal <= sumWeight) return i;
  }
}
